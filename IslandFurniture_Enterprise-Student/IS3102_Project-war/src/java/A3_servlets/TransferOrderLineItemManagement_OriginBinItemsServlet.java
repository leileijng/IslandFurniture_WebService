...
public class TransferOrderLineItemManagement_OriginBinItemsServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            TransferOrderEntity TO = manufacturingWarehouseManagementBean.viewTransferOrder(Long.parseLong(transferOrderId));
            List<ItemStorageBinHelper> listOfLineItems = manufacturingInventoryControlBean.getBinItemList(TO.getOrigin().getId());
            session.setAttribute("listOfLineItems", listOfLineItems);
            response.sendRedirect("A3/transferOrderLineItemManagement.jsp?id=" + transferOrderId);
