...
public class RecipeManagement_AddLineItemRecipeServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            boolean canCreate = restaurantManagementBean.addLineItemToRecipe(SKU, qty, recipeId);

            if (!canCreate) {
                System.out.println("cannot add line item in servlet");
                result = "?errMsg=Error adding Line Item. Please try again.&id=" + recipeId;
                response.sendRedirect("RecipeManagement_LineItemRecipeServlet" + result);
            } else {
                System.out.println("can add line item in servlet");
                result = "?goodMsg=Line Item added successfully.&id=" + recipeId;
                response.sendRedirect("RecipeManagement_LineItemRecipeServlet" + result);
            }
        } catch (Exception ex) {
            out.println(ex);
